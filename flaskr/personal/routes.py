...
def purchase_property():
    ...
    user = User.query.filter_by(user_id=session['userid'], user_associated_guild=int(guild)).first()
    if not user:
        print(1)
        return redirect(url_for("personal.pers_dashboard"))
...
def pers_dashboard(guild):
    ...
    main_account = Account.query.filter_by(id=user.user_main_account).first()
    properties = Property.query.filter_by(property_guild=guild, property_owner_id=user.user_id).all()
    ...
    if request.method == 'POST':
        ...
        if 'account_from' in request.form:
            ...
            if account_from.account_user_id != user.user_id:
                flash('SO CLOSE! But yet so far. Give up messing about with my website - best dev ever.', 'danger')
                return redirect(url_for("personal.pers_dashboard", guild=guild))

            if not account_to or account_to.account_guild_id != user.user_associated_guild:
        ...
        if 'property_name' in request.form:
            ...
            user = User.query.filter_by(user_id=int(new_owner), user_associated_guild=guild).first()
            ...
            flash(
                f'Successfully transferred property.',
                'success')
            return redirect(url_for("personal.pers_dashboard", guild=guild))
...
@personal.route('/properties-dashboard', defaults={'guild': None}, methods=['GET', 'POST'])
...
def prop_dashboard(guild):
    ...
    properties = Property.query.filter_by(property_guild=guild, property_owner_id=None).all()
    server = Server.query.filter_by(guild_id=guild).first()

    return render_template('civ_properties.html', user=user if user is not None else None,
                           logged_in=session['logged_in'], properties=properties,
                           users=User.query.filter_by(user_associated_guild=guild), server=server, )
